...
- **End Season Summary**: A detailed end-of-season modal showing exact task completion counts alongside points to ensure transparency before the season is finalized and historical data is logged. After the admin ends the season, all room members see a read-only summary modal (once per season, tracked in localStorage) with a link to the full season history page.
