...
export function PointLimitModal({
  ...
  // Icon and color config per error type
  const iconConfig = {
    bg: isClaimWarning
  ...
  return (
    <DraggableDrawer onClose={onClose}>
      <div className="text-center py-4">
        <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6 border ${iconConfig.bg}`}>
          <svg className={`w-10 h-10 ${iconConfig.color}`} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            {isClaimWarning ? (
        ...
        <h2 className={`text-2xl font-black text-white mb-3`}>
          {isPointLimit
            ? t("limit_reached" as any)
            : isClaimWarning
            ? t("point_warning" as any)
            : isCooldown
            ? t("cooldown_active" as any)
            : isFutureCycle
            ? t("claim_not_current_cycle" as any)
            : t("too_early" as any)}
        </h2>
