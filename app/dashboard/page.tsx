...
import { useClaimTask, useUnclaimTask, useCompleteTask, useCreateTaskTemplate, useDeleteTask, useEditTaskTemplate, useSkipTask } from "@/hooks/mutations/useTaskMutations";
            ...
            <>
              <div className="absolute right-0 mt-1 w-32 bg-slate-800 border border-white/10 rounded-xl shadow-xl z-50 overflow-hidden animate-fade-in">
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-3 text-sm text-amber-400 hover:bg-amber-500/10 transition-colors flex items-center gap-2 border-t border-white/5"
                >
                  ...
                  {skipTask.isPending ? t("skipping") : t("skip")}
                </button>
                {isAdmin && (
                  <button
                    onClick={() => {
