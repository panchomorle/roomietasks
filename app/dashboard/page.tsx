...
import { useClaimTask, useUnclaimTask, useCompleteTask, useCreateTaskTemplate } from "@/hooks/mutations/useTaskMutations";
import { useState } from "react";
import { DraggableDrawer } from "@/components/DraggableDrawer";

// ─── Task Card ───────────────────────────────────────────────
function TaskCard({ task, userId }: { task: any; userId: string }) {
  ...
  return (
    <div className="group bg-white/[0.04] active:bg-white/[0.08] sm:hover:bg-white/[0.06] border border-white/[0.06] rounded-2xl p-4 sm:p-5 transition-all duration-200 animate-fade-in shadow-sm">
    ...
    <>
      ...
      <div className="space-y-3 sm:space-y-4 pb-20">
        {tasksLoading ? (
          ...
          tasks.map((task) => <TaskCard key={task.id} task={task} userId={user?.id ?? ""} />)
        ) : (
